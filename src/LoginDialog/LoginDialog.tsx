...
export default function LoginDialog(props: { onLogin: (info: AccountInfo) => void }) {
    ...
    const onSubmit = async () => {
        ...
        if (account === '' || password === '') {
            ...
            const data = await fetch(`${backendServerLocation}/api/user/login`, {
                method: 'POST',
                body: JSON.stringify({
                    account: account,
                    password: password,
            ...
            if (data.statusCode === 400) {
                setError(true);
                setErrorMessage('Wrong account or wrong password.');
            } else {
                props.onLogin(data);
            }
        }
    ...
    return (
        <>
            ...
            <Dialog open={isOpen} onBackdropClick={handleClose} onEscapeKeyDown={handleClose} fullWidth maxWidth='xs'>
                ...
                <DialogActions>
                    <Button color='primary' variant='contained' onClick={onSubmit} className={classes.actionbutton}>
                        Confirm
                    </Button>
                    <Button color='secondary' variant='contained' onClick={handleClose} className={classes.actionbutton}>
                        Cancel
                    </Button>
                </DialogActions>
            </Dialog>
